...
// The "judgment quality matters more than raw cost" tier - used for
// preference-profile synthesis (lib/profile.js) and recommendation/Surprise
// Me candidate generation (lib/recommendations.js), as opposed to getModel()
// above (chat, and anything else where speed/cost matters more than squeezing
// out the best possible answer). Profile synthesis has to cross-reference and
// resolve titles across ratings and free-text answers - a cheaper model like
// Haiku doesn't reliably get it right (verified: it kept repeating a user's
// mistranslated title even with an explicit worked example in the prompt).
// Recommendations need the same tier for a different reason: connecting
// specific profile details to a specific title (and, for Surprise Me,
// inventing a genuine non-generic connecting thread to something outside the
// user's usual taste) is exactly the kind of nuanced reasoning where a
// stronger model visibly outperforms Haiku - and at typical prompt sizes for
// this call, the cost difference is a fraction of a cent either way, trivial
// next to what moving title lookups off web search already saved (see
// lib/titles.js). Still swappable via env like CLAUDE_MODEL is.
