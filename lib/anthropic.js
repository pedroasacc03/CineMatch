...
// needs Claude (titles.js, profile.js, recommendations.js, chat.js) goes
// through here, so there's exactly one place that knows how to talk to the
// API, parse its JSON responses, and give a clear error if the API key is missing.
...
// The web search tool definition, per Anthropic's docs
// (https://platform.claude.com/docs/en/agents-and-tools/tool-use/web-search-tool).
// This is what lets the AI Engine look up real, current information about a
// movie/show (streaming availability, RT/IMDb scores) instead of relying on
// its training data, per the "AI web search instead of public APIs" decision
// in the planning doc.
...
// Same as above, but gives Claude the web search tool first, so it can look
// up current information before answering. Used for title lookups and
// recommendation candidate generation.
export async function askClaudeWithWebSearchForJSON({ system, prompt, maxTokens = 2000, maxUses = 3 }) {
  const client = getClaudeClient();
  const message = await client.messages.create({
    model: getModel(),
    max_tokens: maxTokens,
