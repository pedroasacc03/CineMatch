...
/**
 * Whether a user has enough signal for recommendations to actually be
 * personalized instead of generic guesses: at least RATINGS_GOAL watched
 * ratings, AND an AI-built profile actually exists (analyzePreferences runs
 * after every watched rating, so by RATINGS_GOAL this should already be
 * true in practice - checking it explicitly rather than assuming covers the
 * edge case where it hasn't finished yet). Used to gate both "Generate more
 * picks" and "Surprise Me" - see app/api/recommendations/route.js and
...
async function gatherRecommendationContext(userId) {
  ...
  return { user, profile, region, knownTitles };
}
...
export async function generateRecommendations(userId, { movieCount = 2, tvCount = 2 } = {}) {
  const { user, profile, region, knownTitles } = await gatherRecommendationContext(userId);
...
Suggest ${movieCount} movies and ${tvCount} TV shows now.`;
  ...
  const candidates = await askClaudeForJSON({ system, prompt, maxTokens: 1500 });
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      if (!title) continue; // Claude couldn't confirm this was a real title - skip it
      ...
      // One bad candidate (e.g. a web search hiccup) shouldn't fail the whole batch.
      console.error(`Skipping recommendation candidate "${candidate.name}":`, err.message);
    }
  }
  return created;
}
