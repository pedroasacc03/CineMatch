...
Suggest ${movieCount} movies and ${tvCount} TV shows now.`;
  ...
  const rawCandidates = await askClaudeForJSON({ system, prompt, maxTokens: 1500, model: getProfileModel() });
...
Suggest one surprising-but-thoughtful pick now.`;
    ...
    const candidate = await askClaudeForJSON({ system, prompt, maxTokens: 500, model: getProfileModel() });
