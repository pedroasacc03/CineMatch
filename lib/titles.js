...
// The prompt below tells Claude to only ever return a direct image-file URL
// for posterUrl, never a webpage that merely shows one - but web search
// results are inherently unreliable, so this is a defense-in-depth check on
// the actual response: only accept it if it really does look like a direct
// image link. Anything else (a themoviedb.org movie page, an imdb.com title
// page, etc.) gets discarded to null - see components/TitlePoster.js, which
// already falls back to a clean placeholder when there's no posterUrl, so a
// missing poster is a much better outcome than a broken image.
function isDirectImageUrl(url) {
  if (typeof url !== "string") return false;
  try {
    const parsed = new URL(url);
    return parsed.protocol === "https:" && /\.(jpe?g|png|webp)$/i.test(parsed.pathname);
  } catch {
    return false;
  }
}
...
Respond with ONLY a single JSON object - no prose, no markdown code fences - matching exactly this shape:
{
  ...
  "genres": string[],
  "overview": string,
  "posterUrl": string or null (MUST be a direct link to the image file itself, ending in .jpg/.jpeg/.png/.webp -
    e.g. "https://image.tmdb.org/t/p/w500/xyz.jpg" - NEVER a webpage that merely shows or links to the poster
    (e.g. a themoviedb.org/movie/... page, an imdb.com title page, a Google Images result page, a Wikipedia
    page). If you can't find one you're sure is a direct image file URL, use null - a missing poster is far
...
If you cannot confidently identify one specific, real title matching the query, respond with exactly {"error": "not_found"} instead of guessing.`;
  ...
  const data = {
    ...
    posterUrl: isDirectImageUrl(result.posterUrl) ? result.posterUrl : null,
